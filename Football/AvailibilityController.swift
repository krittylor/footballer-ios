...
class AvailibilityController: UIViewController {
    ...
    @IBAction func submitButtonPressed(_ sender: Any) {
        ...
        ref.child("field-schedule").child(fieldId!).child(dateString).setValue(["type" : NOT_AVAILABLE,
                                                                                "userName" : "",
                                                                                "orderId" : "'",
                                                                                "fieldId" : ""])
...
//                }
                ...
                if(isCurrent){
                    ...
                    image.image =  srcImage?.maskWithColor(color: UIColor.red)
...
extension AvailibilityController: HFSwipeViewDelegate {
...
extension AvailibilityController: HFSwipeViewDataSource {
    ...
    func swipeView(_ swipeView: HFSwipeView, viewForIndexPath indexPath: IndexPath) -> UIView {
        ...
        if(swipeView.tag == MONTH_TAG){
            ...
            let dayLabel = UILabel(frame: CGRect(origin: CGPoint(x: 0, y: 16), size: CGSize(width: itemDaySize.width, height: 16)))
